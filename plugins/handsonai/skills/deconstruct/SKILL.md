...
- **Refresh the AI Registry index.** After updating the manifest, refresh `REGISTRY.md` at the workspace root using the regeneration procedure in the `indexing-registry` skill (create the file if it doesn't exist yet). If the manifest has a `notion_url`, also update its Notion row per the `indexing-registry` skill's `references/notion-mirror.md`. Both are best-effort — if the environment can't write to the workspace root or Notion isn't connected, note it and continue; a failed refresh never fails a framework step.
